...
class App(ct.CTk):
    ...
    def __init__(self): 
        ...
        self.frametop = ct.CTkFrame(master= self,fg_color=color2, height=(App.HEIGHT*.8)-60, width=(App.WIDTH)-60, corner_radius=30) 
        ...
        self.framebot = ct.CTkFrame(master= self,fg_color=color2, height=(App.HEIGHT*.2)-30, width=(App.WIDTH)-60, corner_radius=30) 
        self.framebot.grid(row=1, column=0, padx = 30, sticky="nswe")

        self.cmd_capture = ct.CTkButton(master=self.framebot , text="📸", text_color=color5,fg_color=color3,hover_color=color4, 
                            border_width=bwidth, border_color=color1, font=("Roboto Medium", 40), corner_radius= 100 , command=self.saveimg_prompt) #, command=self.saveimg_prompt
...
#this part is for the button logics saving and retaking image
    ...
    def saveimg_prompt(self):
        ...
        self.save = ct.CTkButton(master=self.framebot , text="💾", text_color=color5,fg_color=color3,hover_color=color4, border_width=bwidth, border_color=color1 , font=("Roboto Medium", 40), corner_radius= 100 , command=self.savingimg) #, command=self.saveimg_prompt
        self.save.place(bordermode=tk.INSIDE, relx=0.2, rely=0.5, anchor = tk.CENTER, width=80, height=80) 

        self.titlepart = ct.CTkEntry(master=self.framebot,text_color=color5, fg_color=color3,placeholder_text_color=color5,  border_width=bwidth, border_color=color1, font = ("Roboto Medium", 15), placeholder_text="Filename")
        self.titlepart.place(anchor=tk.CENTER, relx=0.5, rely=0.5, width=210, height=30) 

        self.tryagain = ct.CTkButton(master=self.framebot , text="↻", text_color=color5,fg_color=color3,hover_color=color4,  border_width=bwidth, border_color=color1 , font=("Roboto Medium", 40), corner_radius= 100 , command=self.retake) #, command=self.saveimg_prompt
    ...
    def create_toplevel(self,msg):
        ...
        window.configure(fg_color=color2)
        ...
        label = ct.CTkLabel(window, text=msg, text_color=color1, font=("Roboto Medium", -20)) # text_font=("Roboto Medium", -14)
        label.place(bordermode=tk.INSIDE, relx=0.6, rely=0.4, anchor = tk.CENTER) 
        
        button = ct.CTkButton(window,
                                 width=100,
                                 height=32,
                                  text_color=color5,fg_color=color3,hover_color=color4, 
                                    border_width=bwidth, border_color=color1,
                                 corner_radius=8,
                                 text="Exit",
